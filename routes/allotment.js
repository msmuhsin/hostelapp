...
router.get("/", async (req, res) => {
    try {
        ...
        res
            .status(500)
            .send("Error while getting list of allotment from database");
    }
})
...
router.post('/allocate', async (req, res) => {
    ...
    const { vacancyAvailable, SC_ST_PH_BPL, S1, S3, S5, S7, PG } = allotmentValues;
    ...
    const total_no_SC_ST_PH_BPL_of_S7 = total_SC_ST_PH_BPL_seats - (total_no_SC_ST_PH_BPL_of_S1 + total_no_SC_ST_PH_BPL_of_S3 + total_no_SC_ST_PH_BPL_of_S5 + total_no_SC_ST_PH_BPL_of_PG);
    ...
    const BPL_Seats_S1 = total_no_SC_ST_PH_BPL_of_S1 - (ST_Seats_S1 + PH_Seats_S1 + SC_Seats_S1);
    ...
    const BPL_Seats_S3 = total_no_SC_ST_PH_BPL_of_S3 - (ST_Seats_S3 + PH_Seats_S3 + SC_Seats_S3);
    ...
    const BPL_Seats_S5 = total_no_SC_ST_PH_BPL_of_S5 - (ST_Seats_S5 + PH_Seats_S5 + SC_Seats_S5);
    ...
    const BPL_Seats_S7 = total_no_SC_ST_PH_BPL_of_S7 - (ST_Seats_S7 + PH_Seats_S7 + SC_Seats_S7);

    //PG
    const PGSeats = PG.totalSeats;
    const total_no_of_SC_Seats_in_PG = Math.round(total_no_SC_ST_PH_BPL_of_PG / 4);
    const total_no_of_PH_Seats_in_PG = Math.round(total_no_SC_ST_PH_BPL_of_PG / 4);
    const total_no_of_ST_Seats_in_PG = Math.round(total_no_SC_ST_PH_BPL_of_PG / 4);
    const total_no_of_BPL_Seats_in_PG = total_no_SC_ST_PH_BPL_of_PG - (ST_Seats_PG + PH_Seats_PG + SC_Seats_PG);
    ...
    const PH_Seats_S9 =  total_no_of_PH_Seats_in_PG - (PH_Seats_M1 + PH_Seats_M3);
     ...
     const BPL_Seats_S9 =  total_no_of_BPL_Seats_in_PG - (BPL_Seats_M1 + BPL_Seats_M3);
     


     //? Allocation Of Seats

     const S1_Students = students.filter(student => student.sem === "S1" && student.allotted === false);
     const S3_Students = students.filter(student => student.sem === "S3" && student.allotted === false);
     const S5_Students = students.filter(student => student.sem === "S5" && student.allotted === false);
     const S7_Students = students.filter(student => student.sem === "S7" && student.allotted === false);
    ...
    const allotted_S1_StudentsIds = allotted_S1_Students.map(student => student._id);
    await studentModel.updateMany({ _id: { $in: allotted_S1_StudentsIds } }, { allotted: true });
    await allotmentModel.create({
        MH:  {
            S1: {
